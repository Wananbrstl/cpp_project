...
double calAreaTri(const Point& a, const Point& b, const Point& c) {
    return (cross(a, b) + cross(b, c) + cross(c, a)) * 0.5;
}

/* @param pts counter clockwise 
 * @return the max diameter of convex
 */
// TODO: 没有全部通过
double rotate_clips(vector<Point>& convex) {
    const int N = convex.size();
    if(N < 3) {
        return abs(convex[0], convex[N-1]);
    }
    double ans = 0.0;
    for(int i = 0, j = 2; i < N; ++i) {
        Point a = convex[i], b = convex[(i+1)%N];
        // while(calAreaTri(a, b, convex[j]) < calAreaTri(a, b, convex[(j+1)%N]) - EPS) j = (j+1)%N;
        while (cross(b - a, convex[(j+1)%N] - convex[j]) >= EPS) {
            j = (j+1)%N;
        }
        ans = std::max(ans, std::max(abs(a, convex[j]), abs(b, convex[j])));
    }
    return ans;
}
