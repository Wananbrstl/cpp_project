...
/* tangent line of point and circle */
vector<Point> getTagentLinePC(const Point& pt, const Circle& circle) {
    double d = sqrt(norm(circle.origin-pt)-circle.r*circle.r);
    vector<Point> pts(2);
    getCrossPointCC(circle, Circle(pt, d), pts[0], pts[1]);
    return pts;
}

// TODO: 推导公式
vector<Point> getCommonTangent(const Circle& c1, const Circle& c2) {
    
}


Circle c1[2];
Point query_point, p1, p2;
int main(){
    for(int i = 0; i < 2; ++i)
        scanf("%lf %lf %lf", &c1[i].origin.x, &c1[i].origin.y, &c1[i].r);
    vector<Point> pts = getCommonTangent(c1[0], c1[1]);
    return 0;
}
