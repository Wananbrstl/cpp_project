...
public:
    ...
    int COO(const Point& tgtPoint) const {
        for(int i = 0; i < pt_.size(); ++i) {
            Line line(pt_[i], pt_[(i+1)%pt_.size()]);
            if(toLeftTest(line, tgtPoint) == 0)  {
                if(ccw(line, tgtPoint) == ::ON_SEGMENT)  return Ploygon::ON_SEGMENT;
                else return Ploygon::OTHER;
            }
        }
        return Ploygon::CONTAIN;
    }
...
int main() {
    ...
    std::cout << (plg.isConvex() ? 1 : 0) << std::endl;
    return 0;
}
